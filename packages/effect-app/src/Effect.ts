...
export function catchAllMap<E, A2>(f: (e: E) => A2) {
  return <R, A>(self: Effect.Effect<A, E, R>): Effect.Effect<A2 | A, never, R> =>
    Effect["catch"](self, (err) => Effect.sync(() => f(err)))
}

/**
 * Annotates each log in this scope with the specified log annotation.
 * @deprecated Use Effect.annotateLogs to wrap effects directly in v4
 */
export function annotateLogscoped(_key: string, _value: string): Effect.Effect<void, never, Scope.Scope> {
  return Effect.addFinalizer(() => Effect.void)
}
 ...
 * @deprecated Use Effect.annotateLogs to wrap effects directly in v4
 */
export function annotateLogsScoped(_kvps: Record<string, string>): Effect.Effect<void, never, Scope.Scope> {
  return Effect.addFinalizer(() => Effect.void)
}
