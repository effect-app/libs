...
function makeSQLStoreInt(system: DbSystem, dialect: SQLDialect, jsonColumnType: string) {
  return Effect.fnUntraced(function*({ prefix, rootLevelFieldsWhenAvailable: rootLevelFieldsWhenAvailableDefault }: StorageConfig) {
    ...
    return {
      make: Effect.fnUntraced(function*<IdKey extends keyof Encoded, Encoded extends FieldValues, R = never, E = never>(
        ...
        const ensureRootLevelFieldColumns = Effect.fnUntraced(function*() {
          if (activeRootLevelFieldColumns.length === 0) {
            return
          }
          const existing = yield* exec(`PRAGMA table_info(${JSON.stringify(tableName)})`) as Array<{ name: string }>
          ...
          for (const column of activeRootLevelFieldColumns) {
            if (!existingColumns.has(column.columnName)) {
              yield* exec(
                `ALTER TABLE "${tableName}" ADD COLUMN ${quoteIdentifier(column.columnName)} ${projectedColumnSqlType(dialect, column.kind)}`
              )
            }
            yield* exec(
              `UPDATE "${tableName}" SET ${quoteIdentifier(column.columnName)} = ${projectedColumnBackfillExpr(dialect, column)} WHERE ${quoteIdentifier(column.columnName)} IS NULL`
            )
          }
        })
        ...
        const s: Store<IdKey, Encoded> = {
          ...
          find: (id) =>
            resolveNamespace.pipe(
              Effect.flatMap((ns) => {
                const sqlText = `SELECT id, _etag, data${selectColumnsSql} FROM "${tableName}" WHERE id = ? AND _namespace = ?`
          ...
          sql
            ...
            .pipe(
              ...
              Effect.andThen(Effect.gen(function*() {
                ...
                const existing = yield* exec(ns, `PRAGMA table_info(${JSON.stringify(tableName)})`) as Array<{ name: string }>
                const existingColumns = new Set(existing.map((column) => column.name))
                for (const column of activeRootLevelFieldColumns) {
                  if (!existingColumns.has(column.columnName)) {
                    yield* exec(
                      ns,
                      `ALTER TABLE "${tableName}" ADD COLUMN ${quoteIdentifier(column.columnName)} ${projectedColumnSqlType(sqliteDialect, column.kind)}`
                    )
                  }
                  yield* exec(
                    ns,
                    `UPDATE "${tableName}" SET ${quoteIdentifier(column.columnName)} = ${projectedColumnBackfillExpr(sqliteDialect, column)} WHERE ${quoteIdentifier(column.columnName)} IS NULL`
                  )
                }
              })),
              Effect.orDie,
