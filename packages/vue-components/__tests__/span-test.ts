...
import { TracingLive } from "#lib/observability"
import { DevTools } from "@effect/experimental"
import * as api from "@opentelemetry/api"
import { Layer, pipe } from "effect"
import { Effect, ManagedRuntime, Option } from "effect-app"

const spanName = (span: any | undefined) => span?.name

const runtime = ManagedRuntime.make(
  pipe(process.env["DT"] ? DevTools.layer() : Layer.empty, Layer.provideMerge(TracingLive))
    .pipe(Layer.provideMerge(basicLayer))
)
    ...
    yield* Effect.promise(() =>
      new Promise<void>((res) =>
        setTimeout(() => {
          ...
          console.log("effect promise", spanName(activeSpan), spanName(activeSpan) === "Effect Test")
          res()
        }, 10)
      )
    )
  ...
  api
    ...
    .with(ctx, async () => {
      ...
      await runtime.runPromise(
        Effect
          .gen(function*() {
            console.log("[promise] active span1", spanName(api.trace.getActiveSpan()))
          })
          .pipe(Effect.flatMap(() =>
            Effect
              .gen(function*() {
                console.log("[promise] active span2", spanName(api.trace.getActiveSpan()))
