  ...
  return {
    make: Effect.fnUntraced(function*<IdKey extends keyof Encoded, Encoded extends FieldValues, R = never, E = never>(
      ...
      const resolveNamespace = !config?.allowNamespace
        ...
        : Effect.gen(function*() {
          const namespace = yield* storeId.asEffect()
      ...
      const makeSeedEffect = (ns: string) => {
        ...
        return Effect
          .gen(function*() {
            ...
            yield* InfraLogger.logInfo(`Creating mock data for ${name} (namespace: ${ns})`)
            const m = yield* seed!
            const arr = toNonEmptyArray([...m])
            if (Option.isSome(arr)) {
              yield* bulkSetInternal(arr.value, ns).pipe(Effect.orDie)
            }
            yield* Effect.promise(() =>
      ...
      const bulkSetInternal = Effect.fn("Cosmos.bulkSet [effect-app/infra/Store]", {
        attributes: { "repository.container_id": containerId, "repository.model_name": name }
      })(function*(items: NonEmptyReadonlyArray<PM>, ns: string) {
        yield* Effect.annotateCurrentSpan({ namespace: ns })
        ...
        const batchResult = yield* Effect.forEach(
          ...
          Effect.fnUntraced(function*([i, batch]) {
            const responses = yield* Effect
              .promise(() => bulk(batch.map(([, op]) => op)))
              .pipe(Effect.delay(Duration.millis(i === 0 ? 0 : 150)))
            ...
            if (r2) {
              return yield* Effect.die(
                new CosmosDbOperationError("not able to update records: " + r2.statusCode, responses)
              )
            }
            const r3 = responses.find((x) => x.statusCode > 299 || x.statusCode < 200)
            if (r3) {
              return yield* Effect.die(
                new CosmosDbOperationError("not able to update records: " + r3.statusCode, responses)
              )
            }
      ...
      const bulkSet = (items: NonEmptyReadonlyArray<PM>) =>
        Effect.flatMap(resolveAndSeed, (ns) => bulkSetInternal(items, ns))

      const batchSet = Effect.fn("Cosmos.batchSet [effect-app/infra/Store]", {
        attributes: { "repository.container_id": containerId, "repository.model_name": name }
      })(function*(items: NonEmptyReadonlyArray<PM>) {
        ...
        const x = yield* Effect.promise(() => execBatch(ex, ex[0]?.resourceBody._partitionKey))
        const result = x.result ?? []
        const firstFailed = result.find((x: any) => x.statusCode > 299 || x.statusCode < 200)
        if (firstFailed) {
      ...
      const s: Store<IdKey, Encoded> = {
        queryRaw: Effect.fn("Cosmos.queryRaw [effect-app/infra/Store]", {
          attributes: { "repository.container_id": containerId, "repository.model_name": name }
        })(function*<Out>(query: RawQuery<Encoded, Out>) {
          const q = query.cosmos({ name })
          yield* logQuery(q)
          const ns = yield* resolveAndSeed
          yield* Effect.annotateCurrentSpan({ namespace: ns })
          return yield* Effect.promise(() =>
            container
              ...
              .fetchAll()
              .then(({ resources }) => resources.map((_) => ({ ...defaultValues, ...mapReverseId(_ as any) }) as Out))
          )
        }),

        batchRemove: Effect.fn("Cosmos.batchRemove [effect-app/infra/Store]", {
          attributes: { "repository.container_id": containerId, "repository.model_name": name }
        })(function*(ids: NonEmptyReadonlyArray<Encoded[IdKey]>, partitionKey?: string) {
          const ns = yield* resolveAndSeed
          yield* Effect.annotateCurrentSpan({ namespace: ns })
          yield* Effect.promise(() =>
            execBatch(
        ...
        all: Effect
          .gen(function*() {
            const q = { query: `SELECT * FROM ${name}`, parameters: [] }
            yield* logQuery(q)
            const ns = yield* resolveAndSeed
            yield* Effect.annotateCurrentSpan({ namespace: ns })
            return yield* Effect.promise(() =>
              container
                ...
                .fetchAll()
                .then(({ resources }) => resources.map((_) => ({ ...defaultValues, ...mapReverseId(_) })))
            )
          })
          .pipe(
         ...
         */
        filter: Effect.fn("Cosmos.filter [effect-app/infra/Store]", {
          attributes: { "repository.container_id": containerId, "repository.model_name": name }
        })(function*<U extends keyof Encoded = never>(f: FilterArgs<Encoded, U>) {
          type M = U extends undefined ? Encoded : Pick<Encoded, U>
          const skip = f?.skip
          const limit = f?.limit
          const filter = f.filter
          const q = buildWhereCosmosQuery3(
            idKey,
        ...
        find: Effect.fn("Cosmos.find [effect-app/infra/Store]", {
          attributes: { "repository.container_id": containerId, "repository.model_name": name }
        })(function*(id: Encoded[IdKey]) {
          const ns = yield* resolveAndSeed
        ...
        set: Effect.fn("Cosmos.set [effect-app/infra/Store]", {
          attributes: { "repository.container_id": containerId, "repository.model_name": name }
        })(function*(e: PM) {
          const ns = yield* resolveAndSeed
          yield* Effect.annotateCurrentSpan({ namespace: ns, id: e[idKey] })

          const x = yield* Option.match(Option.fromNullishOr(e._etag), {
            onNone: () =>
              Effect.promise(() =>
                container.items.create({
                  ...mapId(e),
          ...
          if (x.statusCode > 299 || x.statusCode < 200) {
            return yield* Effect.die(
              new CosmosDbOperationError("not able to update record: " + x.statusCode)
            )
          }
          return { ...e, _etag: x.etag }
        }),
        batchSet,
        bulkSet
      }
